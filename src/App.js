...
function App() {
  ...
  const schema = yup.object({
    ...
    phoneNum: yup.string().required("Phone Number is required").matches(/^(0|91)?[6-9][0-9]{9}$/, "Phone Number is not valid"),
    emailId: yup.string().required("Email is required").email("Email is not valid"),
    password: yup.string().required("Password Field is required").min(6, "Password must be at least 6 characters long"),
